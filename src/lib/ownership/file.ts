...
export const getFileOwnership = async (options: { codeowners: string, dir: string, onlyGit: boolean, root?: string } & FromFileOptions): Promise<OwnedFile[]> => {
  ...
  for (const filePath of filePaths) {
    files.push(await OwnedFile.FromPath(filePath, engine, options));
  }
